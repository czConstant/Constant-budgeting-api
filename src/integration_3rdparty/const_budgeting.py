...
class Client(object):
    ...
    def send_pubsub_task(self, task, message_id, data=None):
        ...
        resp = requests.post('{}/budgeting-sub/task/{}/{}/'.format(self.url, task, message_id),
...
client = Client(settings.CONST_BUDGETING_API['URL'],
                settings.CONST_BUDGETING_API['TOKEN'])


class ConstantBudgetingManagement(object):
    @staticmethod
